...
| SD-003 | [x] | P3 | M | **Dynamic OG images per research page.** User chose: sharp + hand-rolled SVG (zero new top-level dependencies — sharp is already available transitively via Astro's image pipeline) and Title + Volume + DOI card content. Built a shared renderer (`src/lib/og-image.ts`, dark-card style matching the existing default) and a static PNG endpoint (`src/pages/research/[slug].png.ts`, pre-rendered at build time — all 10 research pages get their own `/research/<slug>.png`). `SiteLayout` gained an optional `ogImage` prop (falls back to the site default when not passed); `research/[slug].astro` now passes its own image. **Bonus finding while building this:** the site-wide default `public/og-image.png` was itself stale — "Application Development Analyst · PhD Candidate", both facts already fixed everywhere else in text (ID-001, P1-003) but never regenerated as an image asset. Regenerated via a new reusable script (`scripts/generate-default-og.mjs`) with current title/role. | `src/lib/og-image.ts` (new), `src/pages/research/[slug].png.ts` (new), `scripts/generate-default-og.mjs` (new), `src/layouts/SiteLayout.astro`, `src/pages/research/[slug].astro`, `public/og-image.png` |
...
| Date | ID | Item |
|---|---|---|
| 2026-08-24 | LI-008, AWS-MIGRATE-001, SD-003 | **Follow-up sprint** — LI-008: user-verified IIT Student Assistant title on LinkedIn. AWS-MIGRATE-001: new `training` content collection created, AWS Udemy item migrated out of `certifications` via `git mv` (zero visual change, was already hidden). SD-003: dynamic per-page OG images shipped for all 10 research pages (sharp + hand-rolled SVG, zero new dependencies, per user's explicit choice) — new `src/lib/og-image.ts` renderer and `research/[slug].png.ts` static endpoint. Found and fixed in the process: the site-wide default `public/og-image.png` was stale ("Application Development Analyst · PhD Candidate") despite those facts being corrected in text long ago (ID-001, P1-003) — regenerated via new `scripts/generate-default-og.mjs`. |
...
**Remaining ceiling blockers (as of 2026-08-24):** EXT-001 (ResearchGate profile + paper uploads), EXT-005/AN-001 (Search Console + Bing submission — repo-ready, needs your authenticated account), EXT-002/AN-002 (Scholar auto-profile — repo-ready, needs Google to index first), IA-003 (ResearchGate distribution links — depends on EXT-001), EXT-004 (Zenodo `sameAs` in JSON-LD — small code change, still open). CP-005/006/009, UI-013/014, CQ-006, LI-008, AWS-MIGRATE-001, SD-003 all closed as of this date.
